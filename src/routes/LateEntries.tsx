...
function LateEntriesTable(props: { className: () => string }) {
    ...
    const reloadTable = async () => {
        ...
        try {
            ...
            if (
                runs_result &&
                typeof runs_result === "object" &&
                runs_result !== null &&
                "rows" in runs_result &&
                "fields" in runs_result
            ) {
                console.log("Data received:", runs_result)

                // Find field indices
                const fieldMap = new Map()
                ;(runs_result as any).fields.forEach(
                ...
                const transformedEntries: Run[] = (runs_result as any).rows.map(
                    (row: any[], rowIndex: number) => ({
                        ...
                        startTime: row[fieldMap.get("starttimems")] || null,
        ...
        <div class="flex flex-wrap gap-2">
            <For each={classes()}>
                {(cls) => (
                    <Button
                        variant={
                            props.className() === cls ? "default" : "outline"
                        }
