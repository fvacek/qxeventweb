...
import { googleAuthService, type GoogleUser, type GoogleCredentialResponse } from "~/auth/google-auth"
...
const GoogleSignIn: Component<GoogleSignInProps> = (props) => {
  ...
  onMount(async () => {
    if (buttonRef) {
      try {
        ...
        buttonRef.addEventListener('googleSignIn', (event: CustomEvent<GoogleCredentialResponse>) => {
          const user = googleAuthService.parseCredentialResponse(event.detail)
          if (user) {
            handleSignInSuccess(user)
          } else {
            handleSignInError(new Error('Failed to parse Google credential response'))
          }
        })
  ...
  return (
    <div class="google-signin-container">
      ...
      {/* Alternative buttons for different flows - for testing */}
      <div class="google-signin-separator">
      ...
      <div class="space-y-2">
        <button
          ...
          disabled={isLoading()}
          class="google-signin-alt-button"
        >
          {isLoading() ? 'Signing in...' : 'Sign in with Google (Credential Flow)'}
        </button>
        ...
        <button
          ...
          class="google-signin-alt-button"
        >
          {isLoading() ? 'Signing in...' : 'Sign in with Google (Token Flow)'}
        </button>
      </div>
